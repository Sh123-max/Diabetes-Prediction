...
# Loop through models as top-level runs
for name, (model, param_grid) in tuning_models.items():
    with mlflow.start_run(run_name=name) as run:
        mlflow.set_tag("mlflow.runName", name)  # ensures model name shows in Runs column

        exp_id = mlflow.get_experiment_by_name("Healthcare_Model_Comparison").experiment_id
        run_link = f"{mlflow.get_tracking_uri().replace('file://', '')}/#/experiments/{exp_id}/runs/{run.info.run_id}"
